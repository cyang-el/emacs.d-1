...
;;                        mad@nakanishi.math.keio.junet (obsolete)
...
;; I happend to modify both versions of Lars Clausen and Mirko Link
;; for working out well on Emacs 24.
  ...
  (let ((kind 0)
    ...
    (while (< kind 6)
      (setq shiplist (cons `(,(/ kind 2) ,(+ x 10) . ,(+ (* 2 kind) level))
                           shiplist))
      (setq x (+ 4 x))
      (if (> x (+ 40 emvaders-left-margin))
          (progn (setq x emvaders-left-margin) (setq kind (1+ kind)))))
    shiplist
    ))
...
(defmacro emvaders-goto-x-y (x y)
  ...
  `(goto-char (+ (* ,y emvaders-line-length)
                 ,x
                 1))
  ;;`(insert (format "%s.%s" (number-to-string ,x) (number-to-string ,y)))
  )
              ...
              (let ((bomb (emvaders-bomb-at shot)))
                (if bomb
                    (progn
                      ...
                      (setq shot nil))
                  (let ((enemy (emvaders-first 'emvaders-check-shot enemies)))
                    (if enemy
